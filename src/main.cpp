...
#if defined(NDEBUG)
# error "Litedevcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x05dc0ea544a196f31e141cd268c8155a30b2972188ab7d3b6cf083550bfe5275");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Litedevcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Litedevcoin Signed Message:\n";
...
{
    // Litedevcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Litedevcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Litedevcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Litedevcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Litedevcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Litedevcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7d91e7b677947475955b7d84dd8ff0c0edc15feae3ceb2a1cfbf990252b23950");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Use the world time for LDC";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1537283033;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084593569;

        if (fTestNet)
        {
            block.nTime    = 1537282993;
            block.nNonce   = 385410795;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd35705b8ab5b36d22144ff5741a391b4bd2ee1ba811aa213c5e3ab5e9f3e7dab"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc5, 0xb8, 0xda }; // Litedevcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitedevcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitedevcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitedevcoinMiner(CWallet *pwallet)
{
    printf("LitedevcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litedevcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LitedevcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitedevcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitedevcoinMiner, pwallet));
}
